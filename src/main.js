...
function dashboard(){
  ...
  const priorityCards=brief.priorities.map((item,index)=>`<article class="action-card intelligence-card"><div class="action-rank">${index+1}</div><div class="action-copy"><small>${money.format(item.impact)} ESTIMATED ANNUAL IMPACT</small><h3>${item.title}</h3><div class="decision-meta"><span class="decision-score">Decision Score ${item.decisionScore||'—'}</span><span>${item.confidence}% confidence</span><span>${item.timeToValue||'30–90 days'}</span></div></div><button type="button" class="text-button" data-explain="${item.id}">Why? →</button></article>`).join('');
  ...
  const title=`<div class="workspace-title-v3"><div><span class="eyebrow">ATLAS EXECUTIVE WORKSPACE · RELEASE 16</span><strong>${demoMode?demoProfile.company:'Good afternoon, '+escapeHtml(firstName)+'.'}</strong><span>${demoMode?'187 employees · 3 locations · 9,842 transactions':'Dashboard and Atlas, together.'}</span></div><button type="button" class="presentation-toggle" data-action="presentation">${state.presentation?'Exit presentation':'Presentation mode'}</button></div>`;
  return `${title}<section class="workspace-layout-v2 release16-layout">${left}${atlasChat(brief,tx)}</section><footer class="release-footer"><span>Atlas SmartLedger · Release 17</span><span>Evidence-Based Executive Intelligence</span></footer>`;
}
...
function openExplanation(id){
  ...
  document.querySelector('#intelligenceBody').innerHTML=`<div class="decision-hero"><div><small>EXECUTIVE DECISION SCORE</small><strong>${item.decisionScore||'—'}</strong><span>out of 100</span></div><div><small>ESTIMATED ANNUAL IMPACT</small><strong>${money.format(item.impact)}</strong><span>${item.confidence}% confidence</span></div><div><small>TIME TO VALUE</small><strong class="decision-time">${escapeHtml(item.timeToValue||'30–90 days')}</strong><span>${escapeHtml(item.ease||'Medium')} implementation effort</span></div></div><section class="report-section"><h3>Why Atlas surfaced this</h3><p>${escapeHtml(item.why)}</p><ul class="evidence-list">${evidence}</ul></section><section class="report-section"><h3>Next best action</h3><p>${escapeHtml(item.nextStep||'Review the supporting records, confirm the opportunity, and track realized savings.')}</p></section>${rows?`<section class="report-section"><h3>Supporting records</h3><div class="evidence-table-wrap"><table class="evidence-table"><thead><tr><th>Date</th><th>Vendor</th><th>Evidence</th><th>Amount</th></tr></thead><tbody>${rows}</tbody></table></div></section>`:''}`;
  dialog?.showModal();
}
