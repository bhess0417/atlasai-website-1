...
function overnightExecutiveBrief(brief){
  ...
  return `<section class="morning-brief-v2"><div class="brief-copy"><span class="eyebrow">OVERNIGHT EXECUTIVE BRIEF · COMPLETED ${demoProfile.completedAt}</span><h1>Good morning, ${escapeHtml(firstName)}.</h1><p>Atlas reviewed ${demoProfile.annualTransactions.toLocaleString('en-US')} transactions and found ${brief.priorities.length} items worth your attention.</p><div class="brief-actions"><button type="button" class="primary-button fit" data-action="open-report">Open full CEO report</button><button type="button" class="text-button" data-explain="${brief.nextAction?.id||''}">Why this comes first →</button></div></div><div class="brief-score"><small>FINANCIAL HEALTH</small><strong>${brief.healthScore}</strong><span>${brief.cashStatus} outlook · ${brief.confidence}% confidence</span></div><div class="brief-kpis"><div><small>ANNUAL SAVINGS</small><strong>${money.format(brief.annualSavings)}</strong></div><div><small>TOP PRIORITY</small><strong>${brief.nextAction?.title||'Review priorities'}</strong></div><div><small>TRANSACTIONS</small><strong>${demoProfile.annualTransactions.toLocaleString('en-US')}</strong></div></div><div class="brief-priority-row">${top.map((item,index)=>`<button type="button" data-explain="${item.id}"><span>${index+1}</span><div><strong>${item.title}</strong><small>${money.format(item.impact)} annual impact</small></div></button>`).join('')}</div></section>`;
}
function dashboard(){
  ...
  const priorityCards=brief.priorities.map((item,index)=>`<article class="action-card"><div class="action-rank">${index+1}</div><div><small>${money.format(item.impact)} ESTIMATED ANNUAL IMPACT</small><h3>${item.title}</h3><p>${item.confidence}% confidence</p></div><button type="button" class="text-button" data-explain="${item.id}">Review →</button></article>`).join('');
