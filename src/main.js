...
function dashboard(){
  ...
  const title=`<div class="workspace-title-v3"><div><span class="eyebrow">ATLAS EXECUTIVE WORKSPACE · RELEASE 15</span><strong>${demoMode?demoProfile.company:'Good afternoon, '+escapeHtml(firstName)+'.'}</strong><span>${demoMode?'187 employees · 3 locations · 9,842 transactions':'Dashboard and Atlas, together.'}</span></div><button type="button" class="presentation-toggle" data-action="presentation">${state.presentation?'Exit presentation':'Presentation mode'}</button></div>`;
  return `${title}${demoMode?overnightExecutiveBrief(brief):''}<section class="workspace-layout-v2">${left}${atlasChat(brief,tx)}</section><footer class="release-footer"><span>Atlas SmartLedger · Release 15</span><span>First-Screen Executive Workspace</span></footer>`;
}
